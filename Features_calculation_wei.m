...
] = Features_calculation_wei(network)
%% 2019.1, Bocheng Wang

%% local metrics 360 x 360 
strength = (strengths_und(network));
clustering_coef = clustering_coef_wu(network)';
local_efficiency = (efficiency_wei(network, 2)');
betweenness = (betweenness_wei(network))';
...
degree = (degrees_und(network));

%% global metrics 1 x 1
number_of_all_possible_edges = 360 * 359 / 2; %上三角形，不包含对角线
number_of_random_network = 100;

global_efficiency = efficiency_wei(network, 0) * number_of_all_possible_edges / sum(sum(network));
...
for i = 1:number_of_random_network
    ...
    gam_rand = mean(clustering_coef_wu(random_network));
...
%% assortativity
assortativity = assortativity_wei(network, 0);
